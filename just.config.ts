...
export function copyTo() {
  return () => {
      let scriptPath = path.join(__dirname, "dist/scripts/main.js");
      let destination = path.join(__dirname, "BP/scripts/");

      copyFile(scriptPath, destination);
  };
}
function copyFile(src, dest){
  let readstream = fs.createReadStream(src);
  readstream.pipe(fs.createWriteStream(path.join(dest, "main.js")))
}
...
task("copyTo", copyTo());
task("build", series("typescript", "bundle", "copyTo"));
